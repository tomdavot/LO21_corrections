...
namespace TD {
  ...
  public:
    ...
    public:
      ...
      iterator& operator++() { elmt++; return * this; }
      
      iterator operator++(int) { iterator tmp=*this; elmt++; return tmp; }
      
      bool operator==(const iterator& it) const { return elmt==it.elmt; }
      
      bool operator!=(const iterator& it) const { return elmt!=it.elmt; }
      
      T& operator *() const { return *elmt; }
    };

    iterator begin() { return iterator(tab); }
    iterator end() { return iterator(tab+Container<T>::nbEl); }


    class const_iterator {
      const T* elmt;
    public:
      const_iterator(const T* e): elmt(e) {}
      
      const_iterator& operator++() { elmt++; return * this; }
      
      const_iterator operator++(int) { const_iterator tmp=*this; elmt++; return tmp;}
      
      bool operator==(const const_iterator& it) const { return elmt==it.elmt; }
      
      bool operator!=(const const_iterator& it) const { return elmt!=it.elmt; }
      
      const T& operator *() const { return *elmt; }
    };
    
    const_iterator begin() const { return const_iterator(tab); }
    const_iterator end() const { return const_iterator(tab+Container<T>::nbEl); }
